...
def install() -> None:
    ...
    # KUNLUN-MTP: sgl_kernel.utils.is_arch_support_pdl
    def _is_arch_support_pdl():
        return False
    for module in (root, sys.modules["sgl_kernel.utils"]):
        module.is_arch_support_pdl = _is_arch_support_pdl

    # Register custom op schemas so @torch.library.register_fake succeeds.
    _define_custom_ops()

    # Bind the pure-torch DeepSeek-V4 flash_mla shim (get_mla_metadata /
    # flash_mla_with_kvcache / flash_mla_sparse_fwd). Deferred import: it pulls
    # in sglang.srt.* which is only importable after sglang itself is loaded.
    # Called again (idempotent) from register_all() to guarantee binding after
    # sglang import completes.
    try:
        install_flash_mla_shim()
    except Exception:
        pass


def install_flash_mla_shim() -> None:
    """Install the pure-torch flash_mla shim onto the sgl_kernel.flash_mla stub."""
    from sglang_kunlun.kernels.flash_mla_shim import install as _install
