...
class KunlunCompressedTensorsW8A8Int8MoE(CompressedTensorsMoEScheme):
    ...
    def __init__(self, quant_config) -> None:
        ...
        scheme = get_fused_moe_scheme(quant_config)
        self.weight_quant = scheme.get("weights")
        self.input_quant = scheme.get("input_activations")
        weight_strategy = getattr(self.weight_quant, "strategy", None)
        input_strategy = getattr(self.input_quant, "strategy", None)
        weight_strategy = getattr(weight_strategy, "value", weight_strategy)
        input_strategy = getattr(input_strategy, "value", input_strategy)
        supported = (
            self.weight_quant is not None
            and self.input_quant is not None
            and self.weight_quant.num_bits == 8
            and self.input_quant.num_bits == 8
            and self.weight_quant.type == QuantizationType.INT
            and self.input_quant.type == QuantizationType.INT
            and weight_strategy == QuantizationStrategy.CHANNEL.value
            and input_strategy == QuantizationStrategy.TOKEN.value
            and self.weight_quant.symmetric
            and not self.weight_quant.dynamic
            and self.input_quant.dynamic
        )
        if not supported:
            raise ValueError(
                "Kunlun W8A8 MoE requires symmetric channelwise INT8 weights "
                "and dynamic per-token INT8 activation quantization."
            )
        self.moe_runner_config = None
        # logger.info("Using CompressedTensor W8A8Int8")
