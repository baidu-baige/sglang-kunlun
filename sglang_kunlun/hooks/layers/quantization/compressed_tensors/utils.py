...
def get_fused_moe_scheme(quant_config: Any) -> dict[str, Any]:
    """Return the fused MoE scheme, falling back to the linear scheme."""
    quant_config._add_fused_moe_to_target_scheme_map()
    fused_scheme = quant_config.target_scheme_map.get("FusedMoE")
    if fused_scheme is not None:
        return fused_scheme
    return quant_config.target_scheme_map["Linear"]
...
def moe_fc(
    ...
    *,
    ...
    if packed_int4:
        # Only the packed INT4 path needs moe_fc_v3, which carries the
        # tgemm_type / use_pack_int4 knobs.
        kunlun_ops.moe_fc_v3(
            ...
            act=None,
            sort_mode=True,
            tgemm_type="int4_wo_int8",
            use_pack_int4=True,
        )
        return output
    # W8A8 uses plain moe_fc to stay bit-identical with the reference
    # W8A8Int8MoEMethod implementation.
    kunlun_ops.moe_fc(
        x=x,
        x_perchannel_max=x_scale,
        weight=weight,
