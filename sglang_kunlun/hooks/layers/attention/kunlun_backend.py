...
"""Kunlun attention backend implementation adapted from the validated FlashAttention hook implementation."""
...
class KunlunAttentionMetadata:
    ...
    # Pre-translated full->SWA write loc (set per-forward for hybrid SWA pools)
    ...
    swa_spec_metadata: Optional["KunlunAttentionMetadata"] = None

    @dataclass
    class KLXFlashAttentionExtraMetadata:
        """KLXFlashAttentionExtraMetadata"""
        cu_seqlens_q_cpu: torch.Tensor = None
        cu_seqlens_k_cpu: torch.Tensor = None
        host_valid_batch_tensor: torch.Tensor = None
        context_len_tensor: torch.Tensor = None
        host_context_len_tensor: torch.Tensor = None
        device_valid_batch_tensor: Optional[torch.Tensor] = None
        kv_lod_cpu: torch.Tensor = None
        kv_lod_xpu: torch.Tensor = None

    extra_attn_metadata: Optional[KLXFlashAttentionExtraMetadata] = None
...
class KunlunAttentionBackend(AttentionBackend):
    ...
    def __init__(
        ...
        self.forward_metadata: KunlunAttentionMetadata = None
        # extra metadata for handling speculative decoding topk > 1, extended draft decode and verify
        self.forward_metadata_spec_decode_expand: KunlunAttentionMetadata = None
        ...
        # Furthermore, FA4 does not support num_splits=0 with CUDA Graph
    ...
    def _init_forward_metadata_klx(
        self,
        forward_batch: ForwardBatch,
        req_pool_indices: torch.Tensor,
        seq_lens: torch.Tensor,
        forward_metadata: FlashAttentionMetadata,
        spec_info: Optional["SpecInfo"],
        mode: str = "normal_extend",  # "normal_extend" / "decode" / "draft_extend" / "target_verify"
    ):
        bs = forward_batch.batch_size

        if mode == "draft_extend":
            ...
            adjusted_seq_lens_cpu = forward_batch.seq_lens_cpu + num_draft_tokens
            adjusted_seq_lens = forward_batch.seq_lens + num_draft_tokens
        elif mode == "decode" or mode == "normal_extend":
            adjusted_seq_lens_cpu = forward_batch.seq_lens_cpu
            adjusted_seq_lens = forward_batch.seq_lens
        else:
            raise ValueError(f"Unknown mode: {mode}")

        host_valid_batch_tensor = torch.arange(bs, device="cpu", dtype=torch.int32)
        if mode == "draft_extend" or mode == "target_verify":
            device_valid_batch_tensor = host_valid_batch_tensor.to(
                forward_batch.seq_lens.device
            )
        else:
            device_valid_batch_tensor = host_valid_batch_tensor.cuda()
        host_context_len_tensor = adjusted_seq_lens_cpu.int()
        ...
        if spec_info and (
            ...
            _, _, self.qo_indptr, self.custom_mask = (
        ...
        forward_metadata.extra_attn_metadata.host_valid_batch_tensor = (
            host_valid_batch_tensor
        )
        forward_metadata.extra_attn_metadata.device_valid_batch_tensor = (
            device_valid_batch_tensor
        )
        forward_metadata.extra_attn_metadata.host_context_len_tensor = (
            host_context_len_tensor
        )
        forward_metadata.extra_attn_metadata.context_len_tensor = context_len_tensor
        cache_seqlens_int32_cpu = forward_metadata.cache_seqlens_int32.to('cpu')
        forward_metadata.extra_attn_metadata.kv_lod_cpu = torch.zeros(bs + 1, dtype=torch.int32, device="cpu")
        forward_metadata.extra_attn_metadata.kv_lod_cpu[1:] = cache_seqlens_int32_cpu.to(torch.int32).cumsum(dim=0)
        forward_metadata.extra_attn_metadata.kv_lod_xpu = \
            forward_metadata.extra_attn_metadata.kv_lod_cpu.to(forward_metadata.cache_seqlens_int32.device)

    def init_forward_metadata(self, forward_batch: ForwardBatch):
        """Initialize forward metadata hence all layers in the forward pass can reuse it."""
        metadata = KunlunAttentionMetadata()
        seqlens_in_batch = forward_batch.seq_lens
        batch_size = forward_batch.batch_size
        device = seqlens_in_batch.device
        metadata.extra_attn_metadata = (
            KunlunAttentionMetadata.KLXFlashAttentionExtraMetadata()
        )
        ...
        if forward_batch.forward_mode.is_decode_or_idle():
            ...
            if forward_batch.spec_info is not None:
                if self.topk <= 1:
                    ...
                    metadata_expand = KunlunAttentionMetadata()
            ...
            self._init_forward_metadata_klx(
                forward_batch=forward_batch,
                req_pool_indices=forward_batch.req_pool_indices,
                seq_lens=forward_batch.seq_lens,
                forward_metadata=metadata,
                spec_info=None,  # no spec_info
                mode="decode",  # decode
            )
            # TODO: we need to test this part for llama 4 eagle case
            self._maybe_init_local_attn_metadata(forward_batch, metadata, device)
            # metadata.extra_attn_metadata.cu_seqlens_q_cpu = metadata.cu_seqlens_q
            ...
            if self.topk <= 1:
                ...
                metadata.extra_attn_metadata.cu_seqlens_q_cpu = torch.arange(
                    0,
                    batch_size * self.speculative_num_draft_tokens + 1,
                    self.speculative_num_draft_tokens,
                    dtype=torch.int32,
                ...
                metadata_expand = KunlunAttentionMetadata()
                metadata_expand.extra_attn_metadata = (
                    KunlunAttentionMetadata.KLXFlashAttentionExtraMetadata()
                )
            ...
            if any(
                ...
                metadata.extra_attn_metadata.cu_seqlens_q_cpu = torch.nn.functional.pad(
            ...
            if forward_batch.forward_mode == ForwardMode.EXTEND:
                self.cum_q_lod_cpu = torch.zeros(
                ...
                self.cum_kv_lod_cpu[1:] = torch.cumsum(
                    torch.tensor(forward_batch.seq_lens_cpu), dim=0
                )
                metadata.extra_attn_metadata.cu_seqlens_q_cpu = self.cum_q_lod_cpu
                metadata.extra_attn_metadata.cu_seqlens_k_cpu = self.cum_kv_lod_cpu

                if metadata.cu_seqlens_k is not None:
                    metadata.extra_attn_metadata.kv_lod_cpu = metadata.cu_seqlens_k.cpu()
                    metadata.extra_attn_metadata.kv_lod_xpu = metadata.cu_seqlens_k
                else:
                    metadata.extra_attn_metadata.kv_lod_cpu = self.cum_kv_lod_cpu.clone()
                    metadata.extra_attn_metadata.kv_lod_xpu = self.cum_kv_lod_cpu.to(device)
            ...
            if forward_batch.forward_mode.is_draft_extend_v2():
                ...
                metadata.extra_attn_metadata.cu_seqlens_q_cpu = metadata.cu_seqlens_q.cpu()
        ...
        if self.use_sliding_window_kv_pool:
            ...
            # Pre-translate the write loc (full->SWA) once per forward so
            # set_kv_buffer can route SWA-layer writes without re-translating
            # (0.5.14 SWAKVPool.set_kv_buffer requires a pre-translated swa_loc).
    ...
    def forward_extend(
                ...
                if not self.use_mla:
                    self.token_to_kv_pool.set_kv_buffer(
                        ...
                        KVWriteLoc(
                            cache_loc,
                            getattr(
                                self.forward_metadata, "swa_out_cache_loc", None
                            ),
                        ),
                        k,
                        v,
        ...
        # - Different window sizes should be passed in for each q in the first
        #stage of cascade attention, 
        #but FA3 interface doesn't support pass in a list of window sizes.
        # - The overhead of duplicated computation of the common prefix part
        #is small for sliding window layers (seq_len <= window_size),
        #so we can just expand it.
        use_cascade_attn = (
        ...
        if use_local_attn:
            ...
            cu_seqlens_q_cpu = metadata.extra_attn_metadata.cu_seqlens_q_cpu
            cu_seqlens_k_cpu = metadata.extra_attn_metadata.cu_seqlens_k_cpu
        else:
        ...
        if not self.use_mla:
            ...
            if (
                ...
                is_MTP = True
                is_draft_extend = forward_batch.forward_mode.is_draft_extend_v2()
                reshape_q = q.view(-1, 1, layer.tp_q_head_num, layer.head_dim)
                if hasattr(self, "qo_indptr"):
                    is_qo_indptr = True
                batch_size = forward_batch.batch_size
                ...
                # max_num_blocks_per_seq = math.ceil(self.max_context_len / self.page_size)
                q = q.contiguous().view(-1,  layer.tp_q_head_num, layer.head_dim)
                token_num = q.size(0)
                o_select = torch.zeros(token_num, head_num, v_dim, dtype=q.dtype, device=q.device)
                context_lens_cpu = (
                    self.forward_metadata.extra_attn_metadata.host_context_len_tensor
                )
                context_lens_xpu = self.forward_metadata.extra_attn_metadata.context_len_tensor
                if context_lens_cpu is None or context_lens_xpu is None:
                    context_lens_xpu = metadata.cache_seqlens_int32.to(torch.int32).contiguous()
                    context_lens_cpu = context_lens_xpu.cpu().to(torch.int32)
                ...
                if is_draft_extend:
                    kunlun_ops.speculative_attention(
                        ...
                        qlen=max_seqlen_q,
                        max_context_len=page_table.shape[1] * self.page_size,
                        head_num=head_num,
                        head_dim=head_dim,
                        scale=0.0,
                        kv_head_num=kv_head_num,
                        block_size=self.page_size,
                        max_num_blocks_per_seq=page_table.shape[1],
                        max_window_size=layer.sliding_window_size if layer.sliding_window_size is not None else -1,
                        block_tables=page_table,
                        sink=sinks.to(torch.float32) if sinks is not None else None
                        # vo_head_dim=v_dim,
                    )
                    result = o_select.contiguous()
                else: # target verify
                    kunlun_ops.speculative_attention(
                        out=o_select,
                        q=q.view(
                            -1, self.speculative_num_draft_tokens,
                            layer.tp_q_head_num, layer.head_dim
                        ),
                        k_cache=key_cache,
                        v_cache=value_cache,
                        context_lens_cpu=context_lens_cpu,
                        context_lens_xpu=context_lens_xpu,
                        batch_num=batch_size,
                        qlen=self.speculative_num_draft_tokens,
                        max_context_len=page_table.shape[1] * self.page_size,
                        head_num=head_num,
                        head_dim=head_dim,
                        scale=0.0,
                        ...
                        max_window_size=(
                            ...
                            if layer.sliding_window_size is not None else -1
                        ),
                        block_tables=page_table,
                        sink=(
                    ...
                    result = o_select.contiguous()
                result = result.view(-1, head_num, head_dim)[
                    ..., : layer.v_head_dim
                ].reshape(-1, layer.tp_q_head_num * layer.v_head_dim)
                return  result.view(-1, layer.tp_q_head_num * layer.v_head_dim)
            kv_head_num = value_cache.shape[1]
            head_num = layer.tp_q_head_num
            head_dim = layer.head_dim
            k = k.contiguous()
            v = v.view(-1, kv_head_num, v.shape[-1])
            v = torch.nn.functional.pad(v, [0, head_dim - v.shape[-1]], value=0)
            v = v.view(-1, kv_head_num, head_dim)
            o_ = torch.zeros([q.shape[0], layer.tp_q_head_num, layer.head_dim], dtype=q.dtype, device="cuda")

            # Get original LODs from metadata (before alignment)
            orig_cu_seqlens_q_cpu = metadata.extra_attn_metadata.cu_seqlens_q_cpu
            orig_kv_lod_cpu = metadata.extra_attn_metadata.kv_lod_cpu

            # When Q total != KV total, there are prefix tokens in kvcache that
            # the current raw k/v doesn't include. Use paged kvcache mode to read
            # all KV (prefix + extend) from cache.
            # IMPORTANT: Use ORIGINAL LODs for branch decision, NOT aligned ones.
            # _align_lod_to_runtime_tokens uses k.shape[0] which is only the current
            # chunk's tokens, not total KV. It would incorrectly override the real KV total.
            # When Q total != KV total (cross-chunk), all layers should use
            # paged kvcache to read full KV history from cache.
            use_paged_cache = (
                orig_cu_seqlens_q_cpu is not None
                and orig_kv_lod_cpu is not None
                and orig_cu_seqlens_q_cpu[-1] != orig_kv_lod_cpu[-1]
            )

            if use_paged_cache:
                # Paged kvcache mode for chunk 1+ (Q tokens < total KV tokens).
                # For SWA layers: swa_page_table has dummy zero entries at the
                # start (evicted blocks). The operator indexes into page_table
                # based on kv_lod, so we adjust kv_lod to skip the evicted
                # prefix. Must pass swa_left=0 to avoid double-clipping since
                # the window is already encoded in the adjusted kv_lod.
                context_seq_lod_cpu = orig_cu_seqlens_q_cpu
                context_seq_lod_xpu = metadata.cu_seqlens_q
                context_kvlen_lod_cpu = orig_kv_lod_cpu
                context_kvlen_lod_xpu = metadata.extra_attn_metadata.kv_lod_xpu
                page_table = page_table.to(dtype=torch.int32).contiguous()
                context_seq_lod_cpu = context_seq_lod_cpu.to(
                    dtype=torch.int32
                ).contiguous()
                context_seq_lod_xpu = context_seq_lod_xpu.to(
                    dtype=torch.int32
                ).contiguous()
                context_kvlen_lod_cpu = context_kvlen_lod_cpu.to(
                    dtype=torch.int32
                ).contiguous()
                context_kvlen_lod_xpu = context_kvlen_lod_xpu.to(
                    dtype=torch.int32
                ).contiguous()
                _q_view = q.contiguous().view(-1, layer.tp_q_head_num, layer.head_dim)
                kunlun_ops.prefill_attention(
                    ...
                    context_qlen_lod_cpu=context_seq_lod_cpu,
                    context_qlen_lod_xpu=context_seq_lod_xpu,
                    context_kvlen_lod_cpu=context_kvlen_lod_cpu,
                    context_kvlen_lod_xpu=context_kvlen_lod_xpu,
                    alibi_slopes=None,
                    softmax_lse=None,
                    swa_right=window_size[1],
                    swa_left=window_size[0],
                    sink=sinks.to(torch.float32) if sinks is not None else None,
                )
            else:
                # Raw k/v mode: first chunk, all tokens are in k/v.
                # Q == KV, no prefix cache needed.
                context_seq_lod_cpu = (
                    orig_cu_seqlens_q_cpu.to(dtype=torch.int32).contiguous()
                    if orig_cu_seqlens_q_cpu is not None
                    else None
                )
                context_seq_lod_xpu = (
                    metadata.cu_seqlens_q.to(dtype=torch.int32).contiguous()
                    if metadata.cu_seqlens_q is not None
                    else None
                )
                context_kvlen_lod_cpu = (
                    orig_kv_lod_cpu.to(dtype=torch.int32).contiguous()
                    if orig_kv_lod_cpu is not None
                    else None
                )
                context_kvlen_lod_xpu = (
                    metadata.extra_attn_metadata.kv_lod_xpu.to(
                        dtype=torch.int32
                    ).contiguous()
                    if metadata.extra_attn_metadata.kv_lod_xpu is not None
                    else None
                )
                _q_view = q.contiguous().view(-1, layer.tp_q_head_num, layer.head_dim)
                kunlun_ops.prefill_attention(
            ...
            result = o_.view(-1, head_num, head_dim)[
                ..., : layer.v_head_dim
            ].reshape(-1, head_num * layer.v_head_dim)
            return result

            # return (out, softmax_lse) if use_cascade_attn else out
    ...
    def forward_decode(
        ...
        if not self.use_mla:
            ...
            if layer.is_cross_attention:
                ...
                head_dim = layer.head_dim
                max_num_blocks_per_seq = math.ceil(self.max_context_len / self.page_size)
                q = q.contiguous().view(batch_size, 1, layer.tp_q_head_num, layer.head_dim)

                o_select = torch.zeros(batch_size, 1, head_num, head_dim, dtype=q.dtype, device=q.device)
                context_lens_cpu = (
                    self.forward_metadata.extra_attn_metadata.host_context_len_tensor
                )
                context_lens_xpu = self.forward_metadata.extra_attn_metadata.context_len_tensor
                ...
                kunlun_ops.speculative_attention(
                    ...
                    block_tables=page_table,
                    sink=sinks.to(torch.float32) if sinks is not None else None,
                    qlod_cpu=cu_seqlens_q_cpu,
                    qlod_xpu=cu_seqlens_q,
                )
                result = o_select.view(-1, head_num, head_dim)[
                    ..., : layer.v_head_dim
                ].reshape(-1, head_num * layer.v_head_dim).contiguous()
                return result
                if use_cascade_attn:
                    o, softmax_lse, *rest = result
                    o_expand, softmax_lse_expand, *rest_expand = (
    ...
    def init_cuda_graph_state(self, max_bs: int, max_num_tokens: int):
        ...
        # SWA write-target buffer; capture binds a [:num_tokens] view, refilled
        # from the live out_cache_loc before each replay (mirrors FA backend).
        if self.use_sliding_window_kv_pool:
        ...
        self.decode_cuda_graph_metadata = {
            ...
            "context_len_tensor": torch.zeros(
                max_bs, dtype=torch.int32, device=self.device
            ),
            "host_context_len_tensor": torch.zeros(
    ...
    def init_forward_metadata_out_graph(
        ...
        """0.5.14 CUDA-graph metadata entry point (outside graph.capture()).

        0.5.14 replaced the legacy
        ``init_forward_metadata_capture_cuda_graph`` /
        ``init_forward_metadata_replay_cuda_graph`` pair with
        ``init_forward_metadata_out_graph(fb, in_capture)`` +
        ``init_forward_metadata_in_graph(fb)`` (see base_attn_backend.py).

        Kunlun's host-side metadata build is non-graph-recordable, so all of
        it stays here; ``init_forward_metadata_in_graph`` is a no-op. We adapt
        the new ``forward_batch``-based signature onto the existing legacy
        capture/replay implementations, which already set
        ``self.forward_metadata`` (and ``self.forward_metadata_spec_decode_expand``)
        at their end.
        """
        bs = forward_batch.batch_size
        if in_capture:
    ...
    def init_forward_metadata_in_graph(self, forward_batch: ForwardBatch):
        """No-op: Kunlun metadata is built host-side in out_graph."""
        pass
    ...
    def init_forward_metadata_capture_cuda_graph(
        ...
        if forward_mode.is_decode_or_idle():
            if spec_info is not None:
                ...
                metadata.extra_attn_metadata.host_context_len_tensor = (
                    self.decode_cuda_graph_metadata["host_context_len_tensor"][:bs]
                )
                metadata.extra_attn_metadata.context_len_tensor = (
                    self.decode_cuda_graph_metadata["context_len_tensor"][:bs]
                )
            ...
            if self.topk <= 1:
                ...
                # Initialize extra metadata for KLX flash attention (required for TARGET_VERIFY mode)
                metadata.extra_attn_metadata.cu_seqlens_q_cpu = metadata.cu_seqlens_q.cpu()
                # Use pre-allocated buffers so CUDA graph captures the correct memory address.
                # The kernel pointer is fixed at capture time; replay must update the same buffer via copy_().
                metadata.extra_attn_metadata.host_context_len_tensor = (
                    self.target_verify_metadata["host_context_len_tensor"][:bs]
                )
                metadata.extra_attn_metadata.context_len_tensor = (
                    self.target_verify_metadata["context_len_tensor"][:bs]
                )
                metadata.extra_attn_metadata.host_context_len_tensor.copy_(
                    (seq_lens.cpu() + self.speculative_num_draft_tokens).int()
                )
                metadata.extra_attn_metadata.context_len_tensor.copy_(
                    (seq_lens + self.speculative_num_draft_tokens).int()
                )
                ...
                # Initialize extra metadata for KLX flash attention (required for TARGET_VERIFY mode with topk > 1)
                metadata.extra_attn_metadata.cu_seqlens_q_cpu = metadata.cu_seqlens_q.cpu()
                # Use pre-allocated buffers so CUDA graph captures the correct memory address.
                metadata.extra_attn_metadata.host_context_len_tensor = (
                    self.target_verify_metadata_topk_normal["host_context_len_tensor"][:bs]
                )
                metadata.extra_attn_metadata.context_len_tensor = (
                    self.target_verify_metadata_topk_normal["context_len_tensor"][:bs]
                )
                metadata.extra_attn_metadata.host_context_len_tensor.copy_(
                    (seq_lens.cpu() + self.speculative_num_draft_tokens).int()
                )
                ...
                # Also initialize expand metadata buffers for CUDA graph capture
                expand_bs = bs * self.speculative_num_draft_tokens
                metadata_expand.extra_attn_metadata.host_context_len_tensor = (
                    self.target_verify_metadata_topk_expand["host_context_len_tensor"][:expand_bs]
                )
                metadata_expand.extra_attn_metadata.context_len_tensor = (
                    self.target_verify_metadata_topk_expand["context_len_tensor"][:expand_bs]
                )
                metadata_expand.extra_attn_metadata.host_context_len_tensor.copy_(
                    (seq_lens.cpu() + self.speculative_num_draft_tokens).int().repeat_interleave(
                        self.speculative_num_draft_tokens
                    )
                )
                metadata_expand.extra_attn_metadata.context_len_tensor.copy_(
                    (seq_lens + self.speculative_num_draft_tokens).int().repeat_interleave(
                        self.speculative_num_draft_tokens
                    )
                )

                if self.has_swa:
                    metadata_swa = KunlunAttentionMetadata()
            ...
            # Initialize extra metadata for KLX flash attention (required for DRAFT_EXTEND mode).
            # context_len_tensor reuses cache_seqlens_int32 (same values, same pre-allocated buffer).
            # host_context_len_tensor uses a pre-allocated CPU buffer from draft_extend_metadata.
            # Both follow the copy_()-on-replay pattern to satisfy CUDA graph memory address constraints.
            metadata.extra_attn_metadata.cu_seqlens_q_cpu = metadata.cu_seqlens_q.cpu()
            metadata.extra_attn_metadata.context_len_tensor = metadata.cache_seqlens_int32
            metadata.extra_attn_metadata.host_context_len_tensor = (
                self.draft_extend_metadata["host_context_len_tensor"][:bs]
            )
            metadata.extra_attn_metadata.host_context_len_tensor.copy_(seq_lens.cpu().int())
            # 在 metadata.extra_attn_metadata.host_context_len_tensor.copy_(seq_lens.cpu().int()) 之后添加:
            metadata.extra_attn_metadata.kv_lod_cpu = torch.zeros(bs + 1, dtype=torch.int32, device="cpu")
            metadata.extra_attn_metadata.kv_lod_cpu[1:] = seq_lens.cpu().to(torch.int32).cumsum(dim=0)
            metadata.extra_attn_metadata.kv_lod_xpu = metadata.extra_attn_metadata.kv_lod_cpu.to(device)
        ...
        # Bind a static SWA write-loc buffer slice so the captured graph has a
        # stable address; replay refills it from the live out_cache_loc.
        if self.use_sliding_window_kv_pool:
    ...
    def init_forward_metadata_replay_cuda_graph(
        ...
        # Refill the static SWA write-loc buffer from the live out_cache_loc so
        # the captured graph's bound view points at the correct SWA targets.
        ...
        if forward_mode.is_decode_or_idle():
            ...
            if spec_info is not None:
                ...
                metadata.extra_attn_metadata.host_context_len_tensor.copy_(seq_lens_cpu)
                metadata.extra_attn_metadata.context_len_tensor.copy_(seq_lens)
                normal_decode_set_metadata(
                    metadata.cache_seqlens_int32,
                    metadata.cu_seqlens_k,
                    metadata.page_table,
            ...
            if self.topk <= 1:
                ...
                # Update extra metadata for KLX flash attention (required for TARGET_VERIFY mode replay).
                # Must use copy_() to update the pre-allocated buffer in-place.
                # CUDA graph captures the buffer's memory address at capture time; assigning a new tensor (=)
                # would not update what the captured kernel reads.
                metadata.extra_attn_metadata.host_context_len_tensor.copy_(
                    (seq_lens_cpu + self.speculative_num_draft_tokens).int()
                )
                metadata.extra_attn_metadata.context_len_tensor.copy_(
                    (seq_lens + self.speculative_num_draft_tokens).int()
                )
                ...
                # Update extra metadata for KLX flash attention (required for TARGET_VERIFY mode replay with topk > 1).
                # Must use copy_() to update pre-allocated buffers in-place (CUDA graph pointer constraint).
                metadata.extra_attn_metadata.host_context_len_tensor.copy_(
                    (seq_lens_cpu + self.speculative_num_draft_tokens).int()
                )
                metadata.extra_attn_metadata.context_len_tensor.copy_(
                ...
                metadata_expand.extra_attn_metadata.host_context_len_tensor.copy_(
                    (seq_lens_cpu + self.speculative_num_draft_tokens).int().repeat_interleave(
                        self.speculative_num_draft_tokens
                    )
                )
                metadata_expand.extra_attn_metadata.context_len_tensor.copy_(
            ...
            # cache_seqlens_int32 IS context_len_tensor (same buffer); copy_() updates both.
            metadata.cache_seqlens_int32.copy_(seq_lens)
            metadata.extra_attn_metadata.host_context_len_tensor.copy_(seq_lens_cpu.int())
    ...
    def _init_sliding_window_attn_spec_metadata(
        ...
        #     self.page_size == 1
        # ), (
        #     "FlashAttention backend doesn't support topk > 1 speculative"
        #     "decoding with page size > 1 sliding window attention"
        # )
...
class KunlunFlashAttentionMultiStepBackend:
    ...
    def init_forward_metadata_out_graph(
        ...
        """0.5.14 draft CUDA-graph metadata entry point.

        The draft worker always runs DECODE-mode multi-step graphs. Delegate to
        each per-step KunlunAttentionBackend via the legacy capture/replay
        bodies, forcing ForwardMode.DECODE as the original code did.
        """
    ...
    def init_forward_metadata_in_graph(self, forward_batch: ForwardBatch):
        """No-op: metadata built host-side in out_graph."""
        pass
...
FlashAttentionMetadata = KunlunAttentionMetadata
FlashAttentionBackend = KunlunAttentionBackend
FlashAttentionMultiStepBackend = KunlunFlashAttentionMultiStepBackend
