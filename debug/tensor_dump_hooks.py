...
def _register_dsv4_layer0_block_chain(tensor_dumper, model):
    ...
    def dump_attention_stage(name, value):
        add_tensor(name, value)

    self_attn._dsv4_tensor_dump_callback = dump_attention_stage
    ...
    def self_attn_forward_with_dump(self, *args, **kwargs):
        ...
        if attn_backend is not None:
            ...
            # 0.5.17 dispatches operator dumps through a module level callback
            # rather than an attribute on the backend instance.
            from sglang_kunlun.hooks.layers.attention.kunlun_deepseek_v4_backend import (
                set_dsv4_tensor_dump_callback,
            )

            # 0.5.17's backend emits the raw operator argument names; Golden's
            # probes emit the TP-local variants. Map them so paired keys line up.
            operator_name_map = {
                "compressed_attention.input.q": (
                    "compressed_attention.input.q_local"
                ),
                "compressed_attention.input.win_cache": (
                    "compressed_attention.input.win_unique_cache_rows"
                ),
                "compressed_attention.input.extra_cache": (
                    "compressed_attention.input.extra_unique_cache_rows"
                ),
                "compressed_attention.output.out": (
                    "compressed_attention.output.out_local"
                ),
                "compressed_attention.output.max_logits": (
                    "compressed_attention.output.max_logits_local"
                ),
                "compressed_attention.output.lse": (
                    "compressed_attention.output.lse_local"
                ),
            }

            def capture_operator_tensor(name, value):
                add_tensor(f"self_attn.{operator_name_map.get(name, name)}", value)

            set_dsv4_tensor_dump_callback(capture_operator_tensor)
